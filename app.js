// app.js — Bet! MVP API (tokens-only beta)
// Deploy on Render: set env vars DATABASE_URL, JWT_SECRET, PORT=8080
...
app.get('/wallet', auth, async (req, res) => {
  try {
    ...
    const tx = await query('select id, amount, type, ts from transactions where user_id=$1 order by ts desc limit 20', [req.user.id]);
    res.json({ balance: bal, transactions: tx.rows });
  } catch (e) { console.error(e); res.status(500).json({ error: 'wallet failed' }); }
});
...
app.post('/bets', auth, async (req, res) => {
  ...
  try {
    ...
    await client.query('insert into bet_events (bet_id, actor_id, action) values ($1,$2,$3)', [betId, req.user.id, 'CREATED']);
    ...
    if (botType) {
      ...
      await client.query('update bets set opponent_id=$1, side_opponent=$2, status=$3 where id=$4', [botId, sideOpp, 'ACCEPTED', betId]);
      await client.query('insert into bet_events (bet_id, actor_id, action) values ($1,$2,$3)', [betId, botId, 'ACCEPTED']);
      botLine = preLine();
    }
    ...
    console.error(e); res.status(500).json({ error: 'create bet failed' });
...
app.get('/bets', auth, async (req, res) => {
  try {
    ...
    res.json(r.rows);
  } catch (e) { console.error(e); res.status(500).json({ error: 'list bets failed' }); }
});
...
app.post('/bets/:id/accept', auth, async (req, res) => {
  ...
  try {
    ...
    await client.query('update bets set opponent_id=$1, side_opponent=$2, status=$3 where id=$4', [req.user.id, bet.side_creator === 'A' ? 'B' : 'A', 'ACCEPTED', id]);
    await client.query('insert into bet_events (bet_id, actor_id, action) values ($1,$2,$3)', [id, req.user.id, 'ACCEPTED']);

    await client.query('commit');
    res.json({ ok: true });
  } catch (e) {
    await client.query('rollback');
    console.error(e); res.status(500).json({ error: 'accept failed' });
  } finally { client.release(); }
});
...
app.post('/bets/:id/resolve', auth, async (req, res) => {
  ...
  try {
    ...
    the opponentId = bet.opponent_id;
    ...
    await client.query('insert into bet_events (bet_id, actor_id, action, meta) values ($1,$2,$3,$4)', [id, req.user.id, 'RESOLVED', JSON.stringify({ winner })]);
